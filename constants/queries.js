const QUERIES = {
  ...
  SELECT_USER_BY_LOGIN_ID: 'SELECT index_id, ID, PW, name, total_points FROM User WHERE ID = ?',
  SELECT_USER_BY_INDEX_ID: 'SELECT index_id, ID, name, total_points FROM User WHERE index_id = ?',
  SELECT_USER_TOTAL_POINTS: 'SELECT total_points FROM User WHERE index_id = ?',
  SELECT_LEADERBOARD: 'SELECT ID, name, total_points FROM User ORDER BY total_points DESC LIMIT ?',
  INSERT_USER: 'INSERT INTO User (ID, PW, name, total_points) VALUES (?, ?, ?, ?)',
